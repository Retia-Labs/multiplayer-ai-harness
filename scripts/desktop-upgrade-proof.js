...
if (realProvider && (process.env.PLEXUS_DESKTOP_CODEX_PROOF !== '1' || process.env.PLEXUS_DESKTOP_COLLABORATION_PROOF !== '1')) {
  throw new Error('Real-provider upgrade requires explicit desktop Codex and collaboration opt-in.');
}
...
try {
  ...
  const run = spawnSync(process.execPath, [path.join(root, realProvider ? 'test/desktop-smoke.js' : 'test/desktop-lifecycle.js')], {
    cwd: root, stdio: 'inherit', env: { ...process.env, DESKTOP_EXECUTABLE: executable,
      [realProvider ? 'PLEXUS_PROVIDER_UPGRADE_CYCLE' : 'PLEXUS_UPGRADE_CYCLE']: config }
  });
  ...
  const report = { status: run.status === 0 && cycle ? 'pass' : 'fail', platform: process.platform, arch: process.arch,
    os: os.release(), ranAt: new Date().toISOString(), provider: realProvider ? 'codex-host-tools' : 'demo', signedRelease: false,
    from: { file: path.basename(from), sha256: hash(from) }, to: { file: path.basename(to), sha256: hash(to) }, cycle };
  fs.writeFileSync(path.join(evidence, process.platform + '-' + process.arch + (realProvider ? '-provider' : '') + '.json'), JSON.stringify(report, null, 2) + '\n');
  if (report.status !== 'pass') throw new Error('Installed upgrade drill failed. See .artifacts/desktop-upgrade.');
  console.log(realProvider ? 'Installed provider upgrade and collaboration passed; reinstall/rollback measured separately, signatures not qualified.'
    : 'Installed upgrade/reinstall/backup-restore drill passed; demo provider, signatures not qualified.');
