...
## Alpha and beta execution plan — 2026-09-15
...
Internal alpha is macOS + Codex + a browser teammate, qualified on two physical machines through fresh setup, real task creation, encrypted catch-up, delivered correction, scoped approval, diff review, handoff, reconnect and background execution. It does not authorize external distribution or relax beta gates.

Invite-only beta retains macOS and Windows qualification, signed installers and upgrades, supported provider/authentication evidence, independent privacy/control review, hosted-service restore, recovery/revocation/deletion, onboarding/support and content-free measurement. Paid seats are required before charging; free internal use is independent. Claude Code and Cursor remain follow-on releases.

Execution order: reconcile PRs #61/#63/#64 against current main; implement the project workspace and guided setup; qualify and repair the two-machine alpha; complete platform/distribution/service/review gates; begin assisted pilots. Closed foundation issues #2–#18 retain their evidence; new defects receive linked follow-ups.
