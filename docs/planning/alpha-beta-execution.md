...
Prepare one service origin (`app.tryplexus.dev`) with HTTPS/WSS, persistent SQLite storage for the current single-instance architecture, managed process restart, content-safe health checks and the existing deletion-aware backup/restore tooling. The user selected and approved DigitalOcean hosting: a separate Singapore Droplet with 1 vCPU, 1 GB RAM and 25 GB persistent disk at US$6/month before tax and additional usage. The installed revision and artifact qualification are recorded in the deployment runbook; Caddy and Cloudflare DNS provide verified HTTPS at `app.tryplexus.dev`. The relay is running in GitHub authentication mode; real browser consent/callback and session restart persistence are verified. See the [deployment configuration and qualification steps](../operations/hosted-alpha.md). Service restarts interrupt connections and require tested reconnect. Persistent storage does not replace deletion/revocation-preserving backup and restore. No customer-ready claim follows from infrastructure provisioning alone.
...
### Invited Mac distribution exception — 2026-09-19

The founder approved a clearly labelled unnotarized DMG for the first invited Mac alpha. Apple signing/notarization is deferred for this limited release; it remains required for the trusted-distribution milestone. Do not describe the installer as signed, notarized or generally released. Publish the version, architecture and SHA-256 checksum with installation instructions explaining macOS approval. Qualify the actual downloaded artifact and its upgrade behavior. This exception changes only signing/notarization for the invited Mac release; it does not establish Windows support or waive authentication, provider, collaboration, recovery or privacy/control checks.
