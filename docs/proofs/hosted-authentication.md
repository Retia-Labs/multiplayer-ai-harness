...
Design: `shell` + `setup`, existing team rows, fields and buttons; no new tokens or baseline changes. Inspected `.artifacts/hosted-auth/invitations-desktop.png` at 1487×1058 and `invitations-mobile.png`, a focused panel capture at a 390×844 viewport. The existing flat, compact team controls remain; no clipping was observed in the changed panel. These changes were subsequently deployed and included in the `0.2.0-alpha.1` candidate built from `bbe643c`; see the [current deployment and artifact evidence](../operations/hosted-alpha.md).
