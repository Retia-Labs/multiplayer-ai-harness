...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => document.querySelector('#desktop-download-status').textContent.includes('not been published'));
    const dmgName = 'Plexus-0.2.0-alpha.1-mac-arm64-unnotarized.dmg';
    const dmgBytes = Buffer.from('download fixture');
    const dmgSha = require('node:crypto').createHash('sha256').update(dmgBytes).digest('hex');
    fs.writeFileSync(path.join(dir, dmgName), dmgBytes);
    fs.writeFileSync(path.join(dir, 'release.json'), JSON.stringify({ version: '0.2.0-alpha.1', platform: 'darwin', arch: 'arm64', signing: 'no-verified-publisher', notarization: 'not-performed', commit: 'a'.repeat(40), artifacts: [{ file: dmgName, bytes: dmgBytes.length, sha256: dmgSha }] }));
    hub.desktopDownload = new (require('../packages/hub/desktop-download').DesktopDownload)(dir);
    await page.reload();
    await page.locator('#desktop-download-link').waitFor({ state: 'visible' });
    assert.match(await page.locator('#desktop-download-warning').innerText(), /unnotarized/);
    assert.equal(await page.locator('#desktop-download-sha').textContent(), dmgSha);
    const downloaded = page.waitForEvent('download'); await page.locator('#desktop-download-link').click();
    const download = await downloaded;
    assert.equal(download.suggestedFilename(), dmgName);
    assert.deepEqual(fs.readFileSync(await download.path()), dmgBytes);
    await page.locator('#desktop-download-checksum summary').click();
    await page.setViewportSize({ width: 1487, height: 1058 });
    await page.screenshot({ path: path.join(out, 'download-desktop.png') });
    await page.setViewportSize({ width: 390, height: 844 });
    await page.locator('#desktop-download').screenshot({ path: path.join(out, 'download-mobile.png') });
    await page.route('**/api/desktop-release', route => route.fulfill({ status: 503, body: '{}' }));
    await page.reload(); await page.locator('#desktop-download-retry').waitFor({ state: 'visible' });
    await page.unroute('**/api/desktop-release'); await page.locator('#desktop-download-retry').click();
    await page.locator('#desktop-download-link').waitFor({ state: 'visible' });
    console.log('PASS authenticated installer download, exact checksum, unavailable state and failure/retry');
