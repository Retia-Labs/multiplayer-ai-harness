...
test('hosted invitations resolve verified email, require an owner and bind acceptance to the intended account', async t => {
  ...
  const list = () => command(alice.ws, { type: 'team/invite/list', teamId });
  assert.equal((await command(bob.ws, { type: 'team/invite/list', teamId })).type, 'error');
  assert.equal((await list()).invitations[0].status, 'pending');
  assert.equal((await list()).invitations[0].invitee.userId, bob.message.user.id);
  const revoked = await command(alice.ws, { type: 'team/invite', teamId, inviteeEmail: 'bob@example.test' });
  await command(alice.ws, { type: 'team/invite/revoke', code: revoked.invitation.code });
  assert.equal((await list()).invitations.find(i => i.code === revoked.invitation.code).status, 'revoked');
  assert.equal((await command(bob.ws, { type: 'team/invite/accept', code: revoked.invitation.code })).code, 'invitation_revoked');
  ...
  assert.equal((await command(bob.ws, { type: 'team/invite/list', teamId })).type, 'error');
  await command(alice.ws, { type: 'team/invite/revoke', code: invited.invitation.code });
  const receipt = (await list()).invitations.find(i => i.code === invited.invitation.code);
  assert.equal(receipt.status, 'accepted'); assert.equal(receipt.revokedAt, null);
  assert.equal(f.hub.store.membership(teamId, bob.message.user.id).role, 'member');
  const expired = f.hub.store.createInvitation(teamId, alice.message.user.id, bob.message.user.id, 'member', -1);
  await command(alice.ws, { type: 'team/invite/revoke', code: expired.code });
  const expiredReceipt = (await list()).invitations.find(i => i.code === expired.code);
  assert.equal(expiredReceipt.status, 'expired'); assert.equal(expiredReceipt.revokedAt, null);
  assert.equal((await command(bob.ws, { type: 'team/invite/accept', code: expired.code })).code, 'invitation_expired');
  const other = await command(alice.ws, { type: 'team/create', name: 'Other team' });
  assert.deepEqual((await command(alice.ws, { type: 'team/invite/list', teamId: other.team.id })).invitations, []);
});
