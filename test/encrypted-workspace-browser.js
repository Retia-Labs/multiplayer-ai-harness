...
(async () => {
  ...
  assert.ok((await authority.locator('..').innerText()).includes(await alice.evaluate(() => window.__plexus.state.encryptedIdentity.fingerprint)));
  ...
  await recovered.locator('#btn-recovery').click(); await recovered.getByLabel('Recovery key', { exact: true }).fill(replacementKey);
  releaseListing(); await recovered.locator('[data-action="restore-history"]').waitFor();
  assert.equal(await recovered.getByLabel('Recovery key', { exact: true }).inputValue(), replacementKey, 'late backup listing preserves the typed customer key');
