...
Run `npm run check:design` to verify the reference pack and instruction links, and `npm run test:design` for the checker regression tests. Run both checks locally; GitHub Actions is disabled for this repository. These checks cover reference integrity; production visual regression and required review settings still need to be established as the design is adopted.
